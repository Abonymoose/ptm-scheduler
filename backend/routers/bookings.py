...
async def auto_schedule(
    ...
    # Resolve names for ALL requested teachers (including those with no slots)
    ...
    # Fetch available slots: not full, not already booked by this parent
    ...
    # Group available slots by teacher_id
    teacher_slots: dict[str, list] = defaultdict(list)
    for row in rows:
        teacher_slots[str(row.teacher_id)].append(row)

    # Greedy assignment: earliest non-overlapping slot per teacher
    assigned: list[dict] = []
    conflicts: list[str] = []
    ...
    # Book all assigned slots in one transaction with SELECT FOR UPDATE
    booked: list[dict] = []
    if assigned:
        async with db.begin():
            for a in assigned:
