...
export default function AdminDashboard() {
  ...
  const [tab, setTab] = useState('o') // 'o' = overview, 'b' = bookings
  ...
  // Build teacher map from slots
  const teacherMap = {}
  slots.forEach(s => {
    const key = s.teacher_name
    if (!teacherMap[key]) teacherMap[key] = { name: key, email: s.teacher_email || '', venue: s.venue || '', slots: [], booked: 0 }
  ...
  const getInitials = name => name.replace(/^(Ms\.|Mr\.|Dr\.)/, '').trim().split(' ').filter(Boolean).map(w => w[0]).join('').slice(0, 2).toUpperCase()

  // parent history (bookings grouped by parent)
  const parentBookings = {}
  bookings.forEach(b => {
    if (!parentBookings[b.parent_name]) parentBookings[b.parent_name] = []
    parentBookings[b.parent_name].push(b)
  })
      ...
      <div style={{ background: '#fff', border: '1px solid #F4C099', borderRadius: 'clamp(10px,1.5vw,18px)', overflow: 'hidden', width: 'min(96vw,860px)', margin: 'clamp(10px,2vw,24px) auto', display: 'flex', flexDirection: 'column', height: 'calc(100svh - clamp(20px,4vw,48px))' }}>
        ...
        <div style={{ padding: 'clamp(8px,1.2vw,16px) clamp(12px,2vw,24px)', background: '#F47920', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0, gap: 8 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 'clamp(8px,1.2vw,16px)' }}>
            {/* IA white circle avatar */}
            <div style={{ width: 'clamp(28px,3.5vw,44px)', height: 'clamp(28px,3.5vw,44px)', borderRadius: '50%', background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'clamp(9px,1.1vw,13px)', fontWeight: 700, color: '#F47920', flexShrink: 0 }}>IA</div>
            <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 'clamp(8px,1.2vw,14px)' }}>
            <button onClick={() => showToast('Add teacher flow')} style={{ fontSize: 'clamp(11px,1.4vw,15px)', fontWeight: 700, padding: 'clamp(6px,1vw,11px) clamp(12px,1.8vw,20px)', borderRadius: 'clamp(7px,1vw,11px)', background: '#F47920', color: '#fff', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>+ Add teacher</button>
        ...
        {tab === 'o' && (
          <div style={{ flex: 1, overflowY: 'auto', minHeight: 0, display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ padding: 'clamp(8px,1.2vw,14px) clamp(10px,1.5vw,18px)', borderBottom: '1px solid #F4C099', flex: 1 }}>
              ...
              {loading ? <div style={{ padding: 20, color: '#9CA3AF', textAlign: 'center' }}>Loading…</div> : teachers.length === 0 ? (
                <div style={{ padding: 20, color: '#9CA3AF', textAlign: 'center' }}>No teachers yet</div>
              ) : teachers.map((t, i) => {
                ...
                return (
                  <div key={i} style={{ borderRadius: 'clamp(6px,1vw,10px)', border: '1px solid #FDE9D4', marginBottom: 'clamp(4px,.6vw,7px)', overflow: 'hidden' }}>
                    <div onClick={() => setOpenTeacher(isOpen ? null : i)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 'clamp(6px,.9vw,11px) clamp(8px,1.2vw,14px)', background: '#FFF8F3', cursor: 'pointer', userSelect: 'none' }}
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 'clamp(6px,1vw,10px)' }}>
                        <div style={{ width: 'clamp(22px,2.8vw,34px)', height: 'clamp(22px,2.8vw,34px)', borderRadius: '50%', background: '#FFF0E6', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'clamp(8px,1vw,12px)', fontWeight: 700, color: '#F47920', flexShrink: 0 }}>{getInitials(t.name)}</div>
                        <div>
                          <div style={{ fontSize: 'clamp(11px,1.3vw,15px)', fontWeight: 600, color: '#1B3F7A' }}>{t.name}</div>
                          <div style={{ fontSize: 'clamp(8px,1vw,12px)', color: '#9CA3AF' }}>{t.slots.length > 0 ? t.slots[0].subject || '' : ''} · {t.slots.length} slots</div>
                        </div>
                      </div>
                    ...
                    {isOpen && (
                      <div style={{ borderTop: '1px solid #FDE9D4', padding: 'clamp(8px,1.2vw,14px)', background: '#fff' }}>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'clamp(4px,.6vw,8px)', marginBottom: 'clamp(6px,1vw,10px)' }}>
                          {[['Venue', t.venue || '—'], ['Email', t.email || '—'], ['Booked', `${t.booked} / ${t.slots.length}`], ['Free slots', t.slots.length - t.booked]].map(([lbl, val]) => (
                            <div key={lbl}>
                              <div style={{ fontSize: 'clamp(8px,1vw,11px)', color: '#9CA3AF', marginBottom: 1 }}>{lbl}</div>
            ...
            <div style={{ padding: 'clamp(8px,1.2vw,14px) clamp(10px,1.5vw,18px)', borderBottom: '1px solid #F4C099' }}>
              ...
              <div style={{ background: '#FFF0E6', border: '1px solid #F4C099', borderRadius: 'clamp(8px,1.2vw,14px)', padding: 'clamp(10px,1.5vw,18px)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  ...
                  <div style={{ fontSize: 'clamp(18px,2.5vw,28px)', fontWeight: 700, color: '#1B3F7A', letterSpacing: '.08em' }}>INVENT-2026</div>
                </div>
        ...
        {tab === 'b' && (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
            <div style={{ padding: 'clamp(8px,1.2vw,14px)', borderBottom: '1px solid #F4C099', background: '#FFF8F3', display: 'flex', gap: 8, flexShrink: 0 }}>
              <input
                type="text" placeholder="Search parent or teacher..."
                value={search} onChange={e => setSearch(e.target.value)}
                style={{ flex: 1, padding: 'clamp(7px,1vw,12px) clamp(10px,1.5vw,16px)', fontSize: 'clamp(11px,1.4vw,15px)', border: '1.5px solid #F4C099', borderRadius: 'clamp(8px,1vw,12px)', background: '#fff', color: '#1B3F7A', outline: 'none', fontFamily: 'system-ui,sans-serif', boxSizing: 'border-box' }}
            ...
            <div style={{ flex: 1, overflowY: 'auto', minHeight: 0, padding: 'clamp(8px,1.2vw,14px)' }}>
              {loading ? <div style={{ padding: 20, textAlign: 'center', color: '#9CA3AF' }}>Loading…</div> : filteredBookings.length === 0 ? (
                <div style={{ textAlign: 'center', color: '#9CA3AF', fontSize: 'clamp(13px,1.6vw,17px)', marginTop: 40 }}>{search ? 'No results.' : 'No bookings yet.'}</div>
                ...
                return (
                  <div key={bk.id || i}>
                    <div onClick={() => setOpenBooking(isOpen ? null : bk.id)}
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <div style={{ minWidth: 48, textAlign: 'center', background: isCancelled ? '#F3F4F6' : '#FFF0E6', borderRadius: 6, padding: '4px 6px', flexShrink: 0 }}>
                          ...
                          <div style={{ fontSize: 'clamp(9px,1vw,12px)', color: isCancelled ? '#D1D5DB' : '#F4C099', marginTop: 1 }}>{bk.start_time ? fmtDate(bk.start_time) : ''}</div>
                        </div>
                        <div style={{ minWidth: 0 }}>
                          <div style={{ fontSize: 'clamp(12px,1.5vw,15px)', fontWeight: 700, color: '#1B3F7A', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{bk.parent_name}</div>
                          <div style={{ fontSize: 'clamp(10px,1.2vw,13px)', color: '#9CA3AF', marginTop: 1 }}>with {bk.teacher_name}</div>
                        </div>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        <div style={{ fontSize: 'clamp(10px,1.2vw,12px)', fontWeight: 700, padding: '3px 9px', borderRadius: 20, background: isCancelled ? '#F3F4F6' : '#FFF0E6', color: isCancelled ? '#9CA3AF' : '#C45A0A', flexShrink: 0 }}>{isCancelled ? 'Cancelled' : 'Confirmed'}</div>
                    ...
                    {isOpen && (
                      <div style={{ borderTop: '1px solid #FDE9D4', background: '#FFF8F3', border: '1px solid #F47920', borderTop: 'none', borderRadius: '0 0 clamp(5px,.8vw,8px) clamp(5px,.8vw,8px)', marginBottom: 3 }}>
                        {/* Parent card header */}
        ...
        <div style={{ padding: 'clamp(8px,1.2vw,14px) clamp(10px,1.5vw,18px)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#FFF8F3', borderTop: '1px solid #F4C099', flexShrink: 0, flexWrap: 'wrap', gap: 8 }}>
          <span style={{ fontSize: 'clamp(13px,1.6vw,17px)', color: '#C45A0A', fontWeight: 600 }}>{totalBookings} bookings · 09 Apr 2026</span>
